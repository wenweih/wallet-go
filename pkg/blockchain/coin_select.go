...
func CoinSelect(chainHeader int64, txAmount btcutil.Amount, utxos []db.UTXO) ([]db.UTXO, coinset.Coins, error) {
	var coins []coinset.Coin
	for _, utxo := range utxos {
		txHash, err := chainhash.NewHashFromStr(utxo.Txid)
		if err != nil {
      return nil, nil, fmt.Errorf("Convert utxo hexTxid to txHash %s", err)
		}
		amount, err := btcutil.NewAmount(utxo.Amount)
		if err != nil {
      return nil, nil, fmt.Errorf("Convert utxo amount(float64) to btc amount(int64 as Satoshi) %s", err)
		}
	...
	if err != nil {
    return nil, nil, fmt.Errorf("CoinSelect error: %s", err)
	}
